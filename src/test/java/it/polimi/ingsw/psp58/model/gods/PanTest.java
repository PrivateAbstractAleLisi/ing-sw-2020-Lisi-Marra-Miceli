...
public class PanTest {
    ...
    public void setUp() throws Exception {
        ...
        player1.setCard(CardEnum.PAN); //Changed pan
    ...
    @Test
    public void checkWin_panComesDownFromLevel1_shouldReturnNormally() throws CloneNotSupportedException, InvalidMovementException, InvalidBuildException, WinningException {
//        ip.displayIsland();
        card.placeWorker(worker1, 2, 3,island);
        card.placeWorker(worker1B, 1, 3, island);

        island.buildBlock(BlockTypeEnum.LEVEL1, 1, 4);
        card.resetBehaviour();
        card.move(worker1B, 1, 4, island);
        card.resetBehaviour();
        card.move(worker1B, 2, 4, island);

        assertEquals(2, worker1B.getPosition()[0]);
        assertEquals(4, worker1B.getPosition()[1]);

        assertEquals(1, island.getCellCluster(1,4).getCostructionHeight());

    }

    @Test (expected = WinningException.class)
    public void checkWin_panComesDownFromLevel2_shouldThrowException() throws CloneNotSupportedException, InvalidMovementException, InvalidBuildException, WinningException {
        card.placeWorker(worker1B, 1, 3, island);

        island.buildBlock(BlockTypeEnum.LEVEL1, 0, 0);
        island.buildBlock(BlockTypeEnum.LEVEL2, 0, 0);
        island.placeWorker(worker1, 0, 0);
        card.move(worker1, 0, 1, island); //is playing with pan
    }

    @Test (expected = WinningException.class)
    public void checkWin_panComesDownFromLevel3_shouldReturnNormally() throws CloneNotSupportedException, InvalidMovementException, InvalidBuildException, WinningException {

        card.placeWorker(worker1B, 1, 2,island);

        island.buildBlock(BlockTypeEnum.LEVEL1, 1, 4);
        island.buildBlock( BlockTypeEnum.LEVEL2, 1, 4);
        island.buildBlock(BlockTypeEnum.LEVEL3, 1, 4);
        island.placeWorker(worker1, 1, 4);
        card.resetBehaviour();
        card.move(worker1, 1, 3, island); //is playing with pan
    }
}