...
public class MinotaurTest {
    ...
    @Test
    public void move_validSpecialPower_shouldReturnNormally() throws InvalidMovementException, WinningException, CloneNotSupportedException {
        card.placeWorker(worker1, 2,2,island);
        card2.placeWorker(worker2,3 , 3,island);

        card.move(worker1, 3, 3, island);

        assertArrayEquals(new int[]{3, 3},worker1.getPosition() );
        assertArrayEquals(new int[]{4,4},worker2.getPosition() );
    }

    @Test (expected = InvalidMovementException.class)
    public void move_invalidMove_shouldReturnNormally() throws InvalidMovementException, WinningException, CloneNotSupportedException {
