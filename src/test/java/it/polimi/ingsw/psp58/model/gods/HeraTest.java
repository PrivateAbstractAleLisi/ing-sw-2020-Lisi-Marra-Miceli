...
public class HeraTest {
    ...
    public void setUp() throws Exception {
        ...
        player1.setCard(CardEnum.HERA);
        player2.setCard(CardEnum.CHRONUS);
    ...
    //god's specific tests

   @Test
    public void resetBehaviour_opponentTryToWinOnPerimeter_ShouldNotThrowException() throws Exception, WinningException {
        island.buildBlock(BlockTypeEnum.LEVEL1, 0,1);
        island.buildBlock(BlockTypeEnum.LEVEL2, 0,1);
        island.buildBlock(BlockTypeEnum.LEVEL3, 0,1);
        ...
        island.placeWorker(worker1B, 0, 0);
        island.placeWorker(worker1, 3, 2);

        card.resetBehaviour();

        card2.move(worker1B, 0, 1, island);
        assertEquals(2, island.getCellCluster(0, 0).getCostructionHeight());
        assertEquals(3, island.getCellCluster(0, 1).getCostructionHeight());
        assertEquals(worker1B.getPlayerUsername(), island.getCellCluster(0,1).getWorkerOwnerUsername());
        assertEquals(worker1B.getWorkerID(), island.getCellCluster(0,1).getWorkerID());
    }

    @Test (expected = WinningException.class)
    public void resetBehaviour_opponentWinNotOnPerimeter_ShouldThrowException() throws Exception, WinningException {
        island.buildBlock(BlockTypeEnum.LEVEL1, 1,1);
        island.buildBlock(BlockTypeEnum.LEVEL2, 1,1);
        island.buildBlock(BlockTypeEnum.LEVEL3, 1,1);
        ...
        card2.move(worker1B, 1, 1, island);
    }
